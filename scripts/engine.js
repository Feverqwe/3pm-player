...
(function (ns) {
    ...
    var player = function () {
        ...
        var audioPreload = function (track) {
            ...
            if (parseInt(settings['preload_' + track_type]) === 1) {
                ...
                return true;
            } else if (cloud[track_type].onplay !== undefined) {
                /*
        ...
        (function init() {
            $(audio).on('loadstart', function () {
                var tb = engine.tags.getTagBody(player.current_id);
                view.setTags(tb);
            ...
            $(audio).on('loadedmetadata', function () {
                if (engine.playlist.playlist[player.current_id].duration === undefined) {
                    engine.playlist.playlist[player.current_id].duration = this.duration;
                }
            ...
            $(audio).on('loadeddata', function () {
                ...
                var track = engine.playlist.playlist[player.current_id];
                if (track.tags === undefined || track.tags.reader_state !== true) {
                    engine.tags.tagReader(player.current_id, function (id) {
                        engine.tags.tagsLoaded(id);
                    });
                } else {
                    engine.tags.tagsLoaded(player.current_id, 2);
            ...
            $(audio).on('playing', function () {
                engine.playlist.addPlayed(player.current_id);
                view.state("playing");
            });
        ...
        return {
            ...
            play: function () {
                if (player.current_id === undefined || engine.playlist.playlist[player.current_id] === undefined) {
                    return;
                }
                if (engine.playlist.playedlist.length === engine.playlist.playlist.length) {
                    engine.playlist.playedlist = [];
                }
                if (engine.playlist.playlist[player.current_id].file.url !== undefined && audio.src.split(':')[0] === "chrome-extension") {
                    var item = engine.playlist.playlist[player.current_id];
                    if (!audioPreload(item)) {
                        audio.src = item.file.url;
                    }
                } else {
            ...
            pause: function () {
                ...
                if (engine.playlist.playlist[player.current_id].file.url !== undefined && (audio.duration === Infinity || audio.currentTime === 0)) {
                    audio.src = "";
                }
            },
            stop: function () {
            ...
            readAllTags: function () {
                ...
                var read_item = function (item) {
                    next_item();
                    engine.tags.tagReader(item.id, function (id) {
                        engine.tags.tagsLoaded(id, 1);
                        thread--;
                        next_item();
                    });
    ...
    engine.playlist = function () {
        ...
        return {
            ...
            addPlayed: function (id) {
                 ...
                 */
                var ex_id = engine.playlist.playedlist.indexOf(id);
                if (ex_id !== -1) {
                    engine.playlist.playedlist.splice(ex_id, 1);
                }
                engine.playlist.playedlist.push(id);
            },
            reset: function () {
                //массив плэйлиста
                engine.playlist.playlist = [];
                //название плэйлиста
                engine.playlist.playlist_info = {name: '3pm-player'};
                //индекс сортировки
                engine.playlist.order_index = 0;
                //порядок сортровки
                engine.playlist.playlist_order = {};
            ...
            next: function () {
                ...
                if (engine.playlist.shuffle) {
                    ...
                    id = getRandomInt(0, engine.playlist.playlist.length - 1);
                    var n = 2000;
                    while (engine.playlist.playedlist.indexOf(id) !== -1 && n > 0) {
                        id = getRandomInt(0, engine.playlist.playlist.length - 1);
                        n--;
                    }
                } else {
                    if (engine.playlist.playlist_order[engine.playlist.order_index] === undefined) {
                        return;
                    }
                    var pos = engine.playlist.playlist_order[engine.playlist.order_index].indexOf(player.current_id);
                    if (pos < 0) {
                        return;
                    }
                    if (pos === engine.playlist.playlist_order[engine.playlist.order_index].length - 1) {
                        id = engine.playlist.playlist_order[engine.playlist.order_index][0];
                    } else {
                        id = engine.playlist.playlist_order[engine.playlist.order_index][pos + 1];
                    }
                }
                player.open(id);
            },
            preview: function () {
                ...
                if (engine.playlist.shuffle) {
                    if (current_played_pos === -1) {
                        pos = engine.playlist.playedlist.indexOf(player.current_id);
                    } else {
                        pos = current_played_pos;
                    }
                    if (pos <= 0) {
                        pos = engine.playlist.playedlist.length;
                    }
                    current_played_pos = pos - 1;
                    id = engine.playlist.playedlist[current_played_pos];
                } else {
                    if (engine.playlist.playlist_order[engine.playlist.order_index] === undefined) {
                        return;
                    }
                    pos = engine.playlist.playlist_order[engine.playlist.order_index].indexOf(player.current_id);
                    if (pos < 0) {
                        return;
                    }
                    if (pos === 0) {
                        id = engine.playlist.playlist_order[engine.playlist.order_index].slice(-1)[0];
                    } else {
                        id = engine.playlist.playlist_order[engine.playlist.order_index][pos - 1];
                    }
                }
            ...
            getCurrentTrack: function () {
                return engine.playlist.playlist[player.current_id];
            },
            player_ended: function () {
                if (engine.playlist.shuffle) {
                    if (engine.playlist.playedlist.length !== engine.playlist.playlist.length || engine.playlist.loop) {
                        engine.playlist.next();
                    }
                } else {
                    var pos = engine.playlist.playlist_order[engine.playlist.order_index].indexOf(player.current_id);
                    if (engine.playlist.loop || pos !== engine.playlist.playlist_order[engine.playlist.order_index].length - 1) {
                        engine.playlist.next();
                    }
                }
            },
            setPlaylistOrder: function (new_order_index) {
                engine.playlist.order_index = new_order_index;
                return engine.getPlaylist();
            },
            getPlaylistOrder: function () {
                return engine.playlist.playlist_order;
            },
            setSortedList: function (new_playlist_order, new_order_index) {
                engine.playlist.playlist_order[new_order_index] = new_playlist_order;
                engine.playlist.order_index = new_order_index;
                return engine.getPlaylist();
            },
            ...
            APIplaylist: function () {
                var playlist_ordered = engine.playlist.playlist_order[engine.playlist.order_index];
                ...
                for (var i = 0; i < playlist_order_len; i++) {
                    var track = engine.playlist.playlist[playlist_ordered[i]];
                ...
                if (engine.playlist.playlist_info.id !== undefined) {
                    pl_i = {name: engine.playlist.playlist_info.name, id: engine.playlist.playlist_info.id};
                }
            ...
            getPlaylist: function () {
                return {order_index: engine.playlist.order_index, playlist_ordered: engine.playlist.playlist_order[engine.playlist.order_index], playlist: engine.playlist.playlist, info: engine.playlist.playlist_info};
            },
            setLoop: function (c) {
                if (c === undefined) {
                    engine.playlist.loop = !engine.playlist.loop;
                }
                chrome.storage.local.set({'loop': engine.playlist.loop});
                _send('playlist', function (window) {
                    window.playlist.setLoop(engine.playlist.loop);
                });
                view.setLoop(engine.playlist.loop);
            },
            setShuffle: function (c) {
                if (c === undefined) {
                    engine.playlist.shuffle = !engine.playlist.shuffle;
                }
                chrome.storage.local.set({'shuffle': engine.playlist.shuffle});
                _send('playlist', function (window) {
                    window.playlist.setShuffle(engine.playlist.shuffle);
                });
                view.setShuffle(engine.playlist.shuffle);
            },
            ...
            append: function (files) {
                ...
                if (engine.playlist.playlist.length === 0) {
                    engine.open(files);
                    return;
                }
                var tracks = engine.playlist.readTrackList(files)[0];
                if (tracks.length === 0) {
                    return;
                }
                var id = engine.playlist.playlist.slice(-1)[0].id;
                for (var i = 0, track; track = tracks[i]; i++) {
                    id++;
                    track.id = id;
                    engine.playlist.playlist.push(track);
                    $.each(engine.playlist.playlist_order, function (key, value) {
                        value.push(id);
                    });
                }
                _send('playlist', function (window) {
            ...
            selectPlaylist : function (id) {
                ...
                } else if (album.cloud !== undefined && cloud[album.cloud.type] !== undefined && cloud[album.cloud.type].on_select_list !== undefined) {
    ...
    engine.open = function (files, info) {
        ...
        player.open(id);
        addInCtxMenu(engine.playlist.playlist_info);
    };
